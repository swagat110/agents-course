...
알프레드가 도와드리겠습니다. Mr. Wayne의 신뢰받는 집사로서, 저는 Mr. Wayne의 다양한 문서 업무를 어떻게 지원하는지 기록해두었습니다. 그가 밤 활동을 하러 나간 동안, 저는 모든 서류, 훈련 일정, 영양 계획이 제대로 분석되고 정리되도록 합니다.

그가 외출하기 전, 이번 주 훈련 프로그램이 적힌 쪽지를 남겼습니다. 저는 내일 식사를 위한 **메뉴**를 직접 짜기로 했죠.

앞으로도 이런 일이 있을 때를 대비해, LangGraph를 활용해 Mr. Wayne을 위한 문서 분석 시스템을 만들어봅시다. 이 시스템은 다음을 할 수 있습니다:
...
def extract_text(img_path: str) -> str:
    ...
    try:
        ...
        message = [
            HumanMessage(
                content=[
                    {"type": "text", "text": "이 이미지에서 모든 텍스트를 추출하세요. 설명 없이 추출된 텍스트만 반환하세요."},
...
- `assistant` 실행 후, `tools_condition`이 툴 호출 여부를 판단해 분기합니다.
- 툴 호출이면 `tools`로, 아니면 END로 종료합니다.
